...
![An Audit of Teachable's bug reporting lifecycle](/diagrams/teachable-bug-lifecycle.png)
...
**Principle 3: System Clarity > New tooling > new process**
...
!["This table demonstrates 1) how the prior system produced 9 separate categories of prioritization and 2) how we collapsed them into 4."](/diagrams/teachable-bug-priority-overhaul.png)

**Principle 4: Visibility over enforcement**

We defined SLAs (service-level agreements, or in short: expected time-to-resolve based on priority) as a part of this process.
...
*Crucially:* We rolled out the new priority labels before SLA timelines to collect data on how the team naturally met the upcoming SLA guidelines.
We found that the team resolved nearly 85% of `p1` and `p2` bugs, while `p3` and `p4` bugs (whose volume doubled higher priority issues) landed under 40% resolved.

Couple months into rollout, we brought that number from under 40% to above 60%.
